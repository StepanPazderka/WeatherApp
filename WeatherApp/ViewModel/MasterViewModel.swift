...
//        addWeatherRecordsInGrid(latitudeModulo: 45, longitudeModulo: 40)
    ...
    func getWeatherAtCurrentLocation() {
        if didFetchCurrentLocation == false {
            self.getWeatherAt(coordinates: MapViewCoordinates.center)
            didFetchCurrentLocation = true

            DispatchQueue.global(qos: .background).asyncAfter(deadline: .now()+2) { [weak self] in
                guard let coords = self?.MapViewCoordinates else { return }
                
                self?.approximateTemperatureForCurrentLocation(currentCoordinates: coords)
                self?.didFetchCurrentLocation = false
            }
        }
    }
    ...
    func getWeatherAt(coordinates: CLLocationCoordinate2D) {
        service.getWeatherBy(coordinates: coordinates) { result in
            ...
            case .failure(let error):
                ...
                switch error {
                case _:
                    self.alertDescription = error.rawValue
                }
    ...
    /**
        Approximates current temperature based on surrounding temepratures
     */
    func approximateTemperatureForCurrentLocation(currentCoordinates: MKCoordinateRegion) {
        DispatchQueue.global(qos: .userInitiated).async {
            for (index, record) in self.records.enumerated() {
                let distance = getDistanceTo(coordinates: CLLocationCoordinate2D(latitude: CLLocationDegrees(record.coordinates.latitude), longitude: CLLocationDegrees(record.coordinates.longitude)))

                self.records[index].distance = Float(distance)
                    
                DispatchQueue.main.async {
                    print("Record #\(index+1): \(record.temperature) Distance: \(record.distance)")
                }
            }
