...
//        realmDbSyn.delete(WeatherDataDBEntity(from: e)).subscribe().dispose()
    ...
    func getAll() -> AnyPublisher<[WeatherDataDBEntity], Error> {
//        let allEntries = realm.objects(WeatherDataDBEntity.self)
        let allEntries: [WeatherDataDBEntity] = []
        return Result<[WeatherDataDBEntity], Error>.Publisher(allEntries).eraseToAnyPublisher()
//        return RealmPublishers.collection(from: allEntries).map { Array($0) }.eraseToAnyPublisher()
    }
